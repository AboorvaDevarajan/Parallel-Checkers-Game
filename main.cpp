...
/*
 * Indicates the max and min values for tree search Alpha and beta (-/+
 * Infinity)
 */
  ...
  if (myfile.is_open()) {
    while (myfile.good() && (getline(myfile, inputString) != NULL)) {
      if (strncmp(inputString.c_str(), "Test", 4) == 0) {
        ...
        for (int i = 0; i < 8; ++i) {
          if (myfile.good() && (getline(myfile, inputString) != NULL)) {
            for (int j = 0; j < 8; ++j) {
              if ((inputString[j] == '+') || (inputString[j] == 'O')) {
 ...
 *  Initially checks for jumps if any, which has the higher priority and then
 * goes for the single move.
 */
        ...
        if (newTest->checkerBoard[i][j] == 1) {
               ...
               (newTest->checkerBoard[i - 1][j + 1] == -2))) // right jump check
          {
            if (newTest->checkerBoard[i - 2][j + 2] == 0) {
              ...
              nBoard->checkerBoard[i - 2][j + 2] = nBoard->checkerBoard[i][j];
      ...
      for (int j = 0; j < 8; ++j) {
        if (isMax && newTest->checkerBoard[i][j] == 1) {
          ...
          if ((i > 0) && (j < 7) &&
            ...
            Test *nBoard = new Test();
            copyArray(newTest->checkerBoard, nBoard->checkerBoard);
            nBoard->checkerBoard[i - 1][j + 1] = nBoard->checkerBoard[i][j];
            nBoard->checkerBoard[i][j] = 0;
            nBoard->mFrom = (i * 10) + j;
...
int alphaBeta(Test *newTest, int depth, int alpha, int beta, Player *newPlayer,
  ...
  if (newPlayer->isMax) {
    ...
    for (itr = children.begin(); itr < children.end(); ++itr) {
      ...
      *output << "A" << inc_depth - depth << ": " << (*itr)->mFrom << "  to  "
              << (*itr)->mTo << "." << endl;
      alpha = max(alpha, alphaBeta(*itr, depth - 1, alpha, beta, notPlayer,
                                   newPlayer, output));
    ...
    for (itr = children.begin(); itr < children.end(); ++itr) {
      ...
      if (depth == 1) {
        *output << "B" << inc_depth - depth << ": " << (*itr)->mFrom << "  to  "
                << (*itr)->mTo << ";  ";
      } else {
        *output << "B" << inc_depth - depth << ": " << (*itr)->mFrom << "  to  "
                << (*itr)->mTo << "." << endl;
      }
      beta = min(beta, alphaBeta(*itr, depth - 1, alpha, beta, notPlayer,
                                 newPlayer, output));
...
int main(int argc, char **argv) {
  ...
  if (rank == 0) {
    ...
    for (int send_child_count = 1; send_child_count < size; send_child_count++)
      MPI::COMM_WORLD.Send(&children_size, sizeof(int), MPI::INT,
                           send_child_count, 111);
    ...
    for (itr1 = children.begin(); itr1 < children.end(); itr1++) {
      ...
      cout << "\n\nAssigned tag values  " << tag
           << " and its processor: " << proc_to_assign;
      fflush(stdout);
  ...
  if (rank != 0) {
    ...
    int tag_val;
    int child_count;
    Test sample[100]; // here occurs a conflict when two or more processes are
    ...
    for (child_count = 1; child_count <= assigned_child; child_count++) {
      ...
      MPI::COMM_WORLD.Recv(&sample[child_count], sizeof(Test), MPI::BYTE, 0,
                           tag_val);
      vector<Test *> Tests;
  ...
  if (rank == 0) {
    ...
    for (itr_count = 1; itr_count < size; itr_count++) {
      ...
      for (child_count = 1; child_count <= assigned_child; child_count++) {
        ...
        MPI::COMM_WORLD.Recv(&test[test_count], sizeof(Test), MPI::BYTE,
                             itr_count, tag);
